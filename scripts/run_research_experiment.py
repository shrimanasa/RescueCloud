...
==========================
Executes N trials of the attack-detect-contain-recover pipeline.
Writes per-trial metrics to results/research_metrics.csv.
Set TRIAL_COUNT = 18 to reproduce the published benchmark.
"""
...
# Reproducibility
# ---------------------------------------------------------------------------
# To exactly reproduce the published benchmark:
#   TRIAL_COUNT = 18
#   SEED = 42 (set in ml/generate_audit_logs.py)
#   Model: ml/models/isolation_forest.joblib (300 estimators, contamination=0.045)
# Any change to the model or attack simulator will invalidate the benchmark.


# ---------------------------------------------------------------------------
# Result validation
# ---------------------------------------------------------------------------
# After running all trials, validate the CSV:
#   python3 -c "
#   import pandas as pd
#   df = pd.read_csv('results/research_metrics.csv')
#   print(df.describe())
#   print('Clean recoveries:', df['recovery_success'].sum(), '/', len(df))
#   "
# All detection_latency_seconds values should be < 5.0
# All rto_seconds values should be < 30.0 for the paper's claims to hold
