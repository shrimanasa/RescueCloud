"""
init_incident_table.py
======================
Creates the security_incidents table (idempotent: IF NOT EXISTS).

Schema: incident_id, run_id, attack_type, prediction, anomaly_score,
detected_at, estimated_compromise_at, rejected_backup, selected_backup,
rto_seconds, rpo_seconds.

Run once on cluster bootstrap before the research experiment.
"""
...
# Index strategy:
# CREATE INDEX ON security_incidents (detected_at DESC) for time-range queries.
# CREATE INDEX ON security_incidents (attack_type) for attack-type filtering.
# CREATE UNIQUE INDEX ON security_incidents (run_id) enforces idempotency.
