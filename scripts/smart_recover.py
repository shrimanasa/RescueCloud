...
RECOVERY_PORT: int = 5433
REPLAY_POLL_INTERVAL_SECONDS: float = 2.0
MAX_WAL_REPLAY_SECONDS: int = 600
...
# ---------------------------------------------------------------------------
# If WAL replay exceeds MAX_WAL_REPLAY_SECONDS:
#   -> Log the timeout and fall back to snapshot restore (higher RPO)
# If no clean backup exists before compromise time:
#   -> Alert ops immediately; manual intervention required
# If Postgres fails to promote after WAL replay:
#   -> Check pg_log for archive_command failures; verify S3 permissions


# ---------------------------------------------------------------------------
# Dry-run mode
# ---------------------------------------------------------------------------
# Run with --dry-run to simulate the recovery without actually modifying
# the database. This is useful for:
#   1. Validating that a clean backup exists before the compromise time
#   2. Estimating the RPO (time since last clean backup)
#   3. Testing the recovery script in a staging environment
# python3 scripts/smart_recover.py --dry-run --compromise-time '2026-09-01 14:30:00'


# ---------------------------------------------------------------------------
# WAL archive requirements
# ---------------------------------------------------------------------------
# For PITR to succeed, the WAL archive must contain:
#   1. A base backup (pg_basebackup) created BEFORE the compromise time
#   2. All WAL segment files from the base backup to the compromise time
# If any WAL segment is missing, Postgres will stop replay at that point,
# resulting in a higher-than-expected RPO. Monitor S3 for archive gaps.


# ---------------------------------------------------------------------------
# Testing without Postgres
# ---------------------------------------------------------------------------
# Run in simulation mode without a real database:
#   RESCUECLOUD_SIMULATE=1 python3 scripts/smart_recover.py --dry-run
# This validates the backup selection and WAL timeline logic without
# requiring a running Postgres instance. Useful for CI testing.


# ---------------------------------------------------------------------------
# Multi-tenant recovery
# ---------------------------------------------------------------------------
# For a multi-hospital deployment, each hospital has its own:
#   - Postgres instance (separate connection string)
#   - S3 WAL archive bucket (separate ARN)
#   - Backup directory (separate path prefix)
# Pass --hospital-id to smart_recover.py to select the correct resources.
# Single-hospital is the default and only tested configuration.


# ---------------------------------------------------------------------------
# Recovery audit logging
# ---------------------------------------------------------------------------
# Every recovery attempt is logged to security_incidents with:
#   - compromise_time: the target timestamp for WAL replay
#   - selected_backup: the backup used as the base
#   - rto_seconds: measured time from start to service restoration
#   - rpo_seconds: time between last clean backup and compromise time
# These metrics are the source of truth for the research benchmark table.
