...
# ---------------------------------------------------------------------------
HTTP_TIMEOUT_SECONDS: int = 5        # seconds before marking a service as unavailable
MINIO_REQUIRED: bool = False          # set True in prod — MinIO is optional in local dev
BLOCKCHAIN_REQUIRED: bool = False     # set True when Hardhat / Ganache node is expected
DB_CONNECT_TIMEOUT_SECONDS: int = 3   # psycopg2 connect timeout
...
def check_disk_space(path: str = "/", min_gb: float = 2.0) -> None:
    """Warn if available disk space is below min_gb GB."""
    import shutil
    _, _, free = shutil.disk_usage(path)
    free_gb = free / (1024 ** 3)
    if free_gb < min_gb:
        warning("Disk Space", f"{free_gb:.1f} GB free (recommend >= {min_gb} GB)")
    else:
        result("Disk Space", True, f"{free_gb:.1f} GB free on {path}")


def check_env_var(name: str, required: bool = True) -> None:
    """Check that an environment variable is set and non-empty."""
    val = os.getenv(name, "")
    if not val:
        if required:
            result(f"Env: {name}", False, "not set")
        else:
            warning(f"Env: {name}", "not set (optional)")
    else:
        result(f"Env: {name}", True, "set")


def check_model_file() -> None:
    """Verify the Isolation Forest model file exists and is loadable."""
    from pathlib import Path
    model_path = Path(__file__).resolve().parents[1] / 'ml/models/isolation_forest.joblib'
    if not model_path.exists():
        result('ML Model', False, f'{model_path} not found -- run ml/train_isolation_forest.py')
        return
    try:
        import joblib
        joblib.load(model_path)
        result('ML Model', True, f'{model_path.name} loaded OK')
    except Exception as e:
        result('ML Model', False, f'Failed to load: {e}')
