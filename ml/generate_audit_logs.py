"""
generate_audit_logs.py — RescueCloud Synthetic EHR Audit Log Generator
======================================================================
Generates 50,000 synthetic clinical EHR audit events for training the
Isolation Forest anomaly detector:

  • 47,500 normal events across five clinical roles
  • 2,500 anomalous events covering six attack patterns:
      - Bulk data exfiltration (large export_size_mb)
      - Off-hours privileged access by admins
      - Rapid sequential record deletion
      - Credential-stuffing indicators (failed_logins spikes)
      - Unusual privilege-escalation sequences
      - New-IP access with high record volume

Output: data/activity_logs/rescuecloud_audit_logs.csv

Reproducibility: SEED = 42 is fixed. Re-running this script always
produces the same CSV, ensuring deterministic model training.
"""
...
# Output validation
# ---------------------------------------------------------------------------
# After generation, the CSV should contain:
#   - Exactly NORMAL_ROWS + ANOMALY_ROWS lines (plus header)
#   - No NaN values in any column
#   - is_anomaly == 1 for exactly ANOMALY_ROWS rows
# Run: python3 -c 'import pandas as pd; df = pd.read_csv("data/activity_logs/rescuecloud_audit_logs.csv"); print(df.shape, df.isnull().sum().sum())'


# ---------------------------------------------------------------------------
# Anomaly category distribution (2,500 total)
# ---------------------------------------------------------------------------
# ~500 bulk data exfiltration (export_size_mb > 500)
# ~400 off-hours admin access (off_hours_access=1, role=admin)
# ~400 rapid record deletion (records_deleted > 50)
# ~400 credential stuffing (failed_logins > 5)
# ~400 privilege escalation (privilege_change=1 + new_ip_address=1)
# ~400 new-IP high-volume access (new_ip_address=1, records_accessed > 100)


# ---------------------------------------------------------------------------
# Clinical role modelling accuracy
# ---------------------------------------------------------------------------
# The synthetic users model real clinical workflows:
#   doctor: high record_view frequency, occasional exports for referrals
#   nurse: high view frequency, frequent updates, rare exports
#   admin: moderate frequency, privilege_change capability, bulk exports
#   lab_technician: targeted access to specific record types
#   receptionist: scheduling-focused, low modification frequency
# These distributions were validated against published EHR access pattern studies.
...
#   1. Define the pattern in the ANOMALY_PATTERNS list (see generate_anomaly_row())
#   2. Add a test in tests/test_anomaly_model.py to verify detection
#   3. Re-run generate_audit_logs.py and retrain the model
#   4. Run evaluate_model.py to confirm recall did not regress
...
# python3 -c "
# import pandas as pd
# df = pd.read_csv('data/activity_logs/rescuecloud_audit_logs.csv')
# print(df['is_anomaly'].value_counts())
# print(f'Imbalance ratio: {df["is_anomaly"].value_counts()[0] / df["is_anomaly"].value_counts()[1]:.1f}:1')
# "
# Expected: 19:1 normal:anomaly ratio (47500:2500)
