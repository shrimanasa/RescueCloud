...
REQUEST_LOG_SAMPLING_RATE: float = 1.0
...
# Lockdown behaviour
# ---------------------------------------------------------------------------
# When _READ_ONLY_LOCKDOWN is True:
#   - GET requests pass through normally
#   - POST/PUT/DELETE requests return HTTP 503 Service Unavailable
#   - The lockdown is lifted by POST /anomaly/reset (admin only)
# This ensures no further writes can occur during PITR recovery.


# ---------------------------------------------------------------------------
# EHR service API contract
# ---------------------------------------------------------------------------
# POST /patients     - create patient record
# GET  /patients/:id - retrieve patient record
# PUT  /patients/:id - update patient record
# DELETE /patients/:id - delete patient record (soft delete in production)
# GET  /encounters   - list encounters (audit-logged)
# All write operations emit AuditEvent to Redis for Sentinel scoring.


# ---------------------------------------------------------------------------
# Audit event completeness
# ---------------------------------------------------------------------------
# Every request to this service MUST emit an AuditEvent, even if it returns
# an error. Suppressing audit events during errors would create a blind spot
# that attackers could exploit by deliberately triggering errors to avoid
# anomaly detection. The middleware handles this before the route handler runs.


# ---------------------------------------------------------------------------
# Synthetic data mode
# ---------------------------------------------------------------------------
# For development and testing, set EHR_SYNTHETIC_MODE=1 to use the Synthea
# clinical dataset instead of real patient data. In this mode:
#   - All patient IDs are fictional
#   - No HIPAA restrictions apply to the data
#   - Load test data: python3 database/import_synthea.py
