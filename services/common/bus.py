...
PUBLISH_SOCKET_TIMEOUT: float = 2.0
LOCAL_ECHO_ALWAYS: bool = False
...
# Drop duplicate (channel, event_id) pairs within this window during reconnect storms.
DEDUP_CACHE_TTL_SECONDS: float = 5.0


# ---------------------------------------------------------------------------
# Performance notes
# ---------------------------------------------------------------------------
# The EventBus uses a single background thread per subscription to listen
# for Redis messages. For high-throughput deployments, consider using
# asyncio-redis with multiple consumer groups instead of blocking pub/sub.
# The current design is sufficient for typical hospital event volumes
# (<1000 audit events/second at peak).


# ---------------------------------------------------------------------------
# Testing with fakeredis
# ---------------------------------------------------------------------------
# To unit test code that uses EventBus without a real Redis instance:
#   import fakeredis
#   bus = EventBus()
#   bus._redis_client = fakeredis.FakeRedis()
#   bus._connected = True
# This allows testing publish/subscribe without a running Redis server.


# ---------------------------------------------------------------------------
# Observability
# ---------------------------------------------------------------------------
# The EventBus does not currently emit Prometheus metrics.
# Future work: add counters for:
#   - Messages published per channel
#   - Messages delivered to subscribers
#   - Redis connection failures and fallback activations
# This would allow Grafana dashboards to show event throughput and bus health.
