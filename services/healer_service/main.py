...
WAL_REPLAY_TIMEOUT_SECONDS: int = int(os.getenv("WAL_REPLAY_TIMEOUT", "600"))
POSTGRES_READY_POLL_INTERVAL: float = 2.0
HEALTH_CHECK_MAX_RETRIES: int = 10


# ---------------------------------------------------------------------------
# Recovery sequence
# ---------------------------------------------------------------------------
# 1. Receive CandidateBackupEvent with backup path and target recovery time
# 2. Stop EHR service write path (set _READ_ONLY_LOCKDOWN via EHR API)
# 3. Force WAL segment switch (pg_switch_wal) to flush in-flight transactions
# 4. Restore base backup to recovery Postgres instance on RECOVERY_PORT
# 5. Write recovery.signal with recovery_target_time
# 6. Start recovery Postgres; poll until promoted to read-write
# 7. Health-check restored database; publish RecoveryCompletedEvent
# 8. Promote recovery instance to primary; update connection strings


# ---------------------------------------------------------------------------
# Post-recovery validation
# ---------------------------------------------------------------------------
# After WAL replay completes, the Healer verifies data integrity by:
#   1. Counting records in key EHR tables (patients, encounters, medications)
#   2. Comparing against pre-attack snapshot stored in security_incidents
#   3. Running a checksum query on the last 1000 records modified before compromise
# Only if all checks pass is the service promoted to primary and circuit breaker reset.


# ---------------------------------------------------------------------------
# CloudNativePG integration notes
# ---------------------------------------------------------------------------
# In Kubernetes with CloudNativePG:
#   - The Healer does not need to manage Postgres promotion manually
#   - CloudNativePG automatically promotes a standby when the primary fails
#   - The Healer's role is limited to: initiating targeted WAL recovery
#     and updating the connection string to point to the restored instance
# In bare-metal/Docker deployments, the Healer manages full promotion.


# ---------------------------------------------------------------------------
# Testing recovery in staging
# ---------------------------------------------------------------------------
# Before going to production, run a full recovery drill in staging:
#   1. Insert synthetic patient records
#   2. Run a mock attack: python3 scripts/run_mock_attack.py
#   3. Verify circuit breaker trips
#   4. Allow Healer to complete PITR recovery
#   5. Verify all synthetic records are present post-recovery
#   6. Measure and record actual RTO and RPO
# Repeat this drill quarterly to maintain operational readiness.
