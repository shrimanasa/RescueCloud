...
THREAT_ALERT_THRESHOLD: float = float(os.getenv("THREAT_ALERT_THRESHOLD", "0.75"))
ESCALATION_MIN_EVENTS: int = int(os.getenv("ESCALATION_MIN_EVENTS", "2"))
ENTROPY_BLEND_WEIGHT: float = float(os.getenv("ENTROPY_BLEND_WEIGHT", "0.3"))  # 0.0 = pure ML

# Entropy scoring: blended = 0.7 * ml_score + ENTROPY_BLEND_WEIGHT * entropy_score


# ---------------------------------------------------------------------------
# Auditor decision flow
# ---------------------------------------------------------------------------
# 1. Receive ThreatDetectedEvent from Sentinel via Redis
# 2. List all backups in backups/ directory
# 3. Filter to pre-compromise candidates (temporal filter)
# 4. Verify each candidate SHA-256 against on-chain BackupLedger
# 5. Select the most recent passing candidate
# 6. Publish CandidateBackupEvent with selected backup metadata


# ---------------------------------------------------------------------------
# Auditor availability
# ---------------------------------------------------------------------------
# The Auditor is stateless — it can be restarted without losing state.
# If the Auditor misses a ThreatDetectedEvent (e.g. during a restart),
# the Gateway Commander will re-publish the event after a timeout.
# Set RECOVERY_TRIGGER_TIMEOUT_SECONDS in the Gateway to control this window.


# ---------------------------------------------------------------------------
# Backup storage path convention
# ---------------------------------------------------------------------------
# Backup files follow naming convention:
#   rescuecloud_YYYY-MM-DD_HH-MM-SS.sql
# This allows timestamp extraction without parsing file metadata.
# The blockchain record maps backup name -> SHA-256 hash for integrity.


# ---------------------------------------------------------------------------
# Backup directory monitoring
# ---------------------------------------------------------------------------
# The Auditor checks BACKUP_DIR on every CandidateBackupEvent trigger.
# If BACKUP_DIR is empty, the Auditor publishes a RECOVERY_FAILED event.
# Ensure the backup sidecar (scripts/backup.sh) is running and healthy
# before deploying the Auditor service to production.


# ---------------------------------------------------------------------------
# Performance tuning
# ---------------------------------------------------------------------------
# Backup integrity verification (SHA-256) is the Auditor's bottleneck.
# For large backup files (>1 GB), verification can take 2-5 seconds.
# To reduce latency, consider caching verified hashes in Redis:
#   Key: sha256_cache:{backup_filename}
#   Value: {hash: '...', verified_at: timestamp}
#   TTL: 3600 seconds (re-verify hourly)
