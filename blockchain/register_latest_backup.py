...
# Automation note
# ---------------------------------------------------------------------------
# This script is called by the backup sidecar (scripts/backup.sh) immediately
# after each successful pg_dump cycle. The typical flow:
#   1. pg_dump completes -> backup file written to backups/
#   2. backup.sh calls: python3 blockchain/register_latest_backup.py
#   3. SHA-256 hash is written to BackupLedger on-chain
#   4. MINIO upload begins in parallel
...
#   1. Log the error with the backup file path and SHA-256 hash
#   2. Queue the registration for retry (up to 3 attempts)
#   3. If all retries fail, alert ops — backup exists but is NOT on-chain
#   4. The Auditor will REJECT this backup until it is successfully registered
