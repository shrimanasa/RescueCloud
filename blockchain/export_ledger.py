...
export_ledger.py
================
Reads all BackupLedger records and writes JSON for offline audit.
Pass --format csv to emit CSV instead (useful for spreadsheet audits).
"""
...
#   - Excel/Google Sheets for compliance audits
#   - Splunk/ELK for SIEM integration
#   - Python pandas for statistical analysis
# Example: python3 -c 'import pandas as pd; df = pd.read_json("ledger.json"); print(df.describe())'


# ---------------------------------------------------------------------------
# Ledger migration
# ---------------------------------------------------------------------------
# To migrate the ledger to a new blockchain node:
#   1. Export: python3 blockchain/export_ledger.py --output ledger_export.json
#   2. Deploy new contract: python3 blockchain/deploy_and_import_ledger.py --import ledger_export.json
#   3. Verify: python3 blockchain/read_ledger.py --verify
#   4. Update BACKUP_LEDGER_ADDRESS in .env with the new contract address
