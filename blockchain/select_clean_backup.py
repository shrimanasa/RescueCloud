...
# Backup selection: temporal filter -> integrity SHA-256 check -> recency sort.
# Reject post-compromise backups; verify on-chain hash; pick most recent passing.


# Edge case: if ALL backups fail integrity check, selection returns None.
# The Auditor must handle this by publishing a RECOVERY_FAILED event and
# alerting the on-call engineer — there is no automated fallback beyond this.


# Logging: every selection decision is logged to stderr with:
#   - Number of candidates before temporal filter
#   - Number remaining after temporal filter
#   - Number remaining after integrity check
#   - Name of selected backup (or NONE if all failed)
# This log is captured in security_incidents for forensic review.


# ---------------------------------------------------------------------------
# Testing the selection algorithm
# ---------------------------------------------------------------------------
# To test backup selection without a blockchain connection:
#   1. Create dummy backup files with timestamps before/after a test compromise time
#   2. Mock Web3 with a fixture that returns known SHA-256 hashes
#   3. Verify the correct backup is selected (most recent pre-compromise, verified)
# See tests/test_services.py for the integration test stub.
